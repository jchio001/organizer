...
class PlaceSelectionActivity : AppCompatActivity() {
    ...
    lateinit var autoCompleteAdapter: AutoCompleteAdapter<Place>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        autoCompleteAdapter.itemConsumer = object: Consumer<Place> {
            override fun accept(place: Place) {
                val intent = Intent()
                intent.putExtra("place", place)
                this@PlaceSelectionActivity.setResult(Activity.RESULT_OK, intent)
                finish();
            }
        }
        ...
        disposable = querySubject
            ...
            .switchMap {
                if (!it.isEmpty()) {
                    return@switchMap ClientManager.get()
                        .organizerClient.getPlaces(it.toString(), null)
                } else {
                    return@switchMap Observable.never<ApiUIModel<List<Place>>>()
                }
            }
            .subscribe{
                when (it.state) {
